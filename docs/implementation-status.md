...
Phase 8 has begun with a server-only operations data layer and protected dashboard, submission queue summary, and health/spend view. The dashboard reports review counts, low-confidence extractions, product conflicts, stale evidence, provider failures, privacy-safe demand signals, recent submissions, and 30-day AI calls/cost/latency. Every admin page now checks the current allowlisted role before loading data, the admin health endpoint requires an active admin membership rather than authentication alone, and role-denial tests cover inactive users and reviewer/publisher separation. Migration `0024` adds the first transactional moderation boundary: reviewers can move an evaluated submission into review, approval, or rejection only with an expected `updated_at` value, and the same transaction writes a minimal before/after audit record. The route also requires explicit confirmation and maps concurrent edits to a refresh-required conflict. `0024` has not been applied to staging or production and its pgTAP test has not run on this Docker-less host. Conflict-safe product/formulation workflows, ruleset operations, school-policy operations, full queue detail, and the moderation UI remain outstanding.
