...
OFF lookup coverage is complete for all 60 newly shortlisted candidates. The final
...
The first repeated OTP test also reached Supabase's fixed built-in test-mailer ceiling and
returned `429 email rate limit exceeded`. The reviewer screen now explains that temporary
condition and permits entry of a recently received code without sending another email. A
custom SMTP provider remains a production-launch prerequisite for reliable authentication
delivery.
