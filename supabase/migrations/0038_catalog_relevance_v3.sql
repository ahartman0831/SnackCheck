-- Support the versioned broad-category policy without rewriting historical runs.
-- Each assessment/shortlist run must match its candidates' policy version.
-- Existing v2 evidence queues remain eligible during the staged v3 rollout.
...
begin
  ...
  if assessment_count not between 1 and 1000
    ...
    or coalesce(p_run ->> 'algorithmVersion', '') not in ('classroom-use-v2', 'classroom-use-v3')
  ...
  if exists (
    ...
    where pg_catalog.jsonb_typeof(assessment) is distinct from 'object'
      ...
      or (assessment ->> 'version') is distinct from (p_run ->> 'algorithmVersion')
...
begin
  ...
  if prior_audit_count > 0 then
    if prior_audit_count <> pg_catalog.cardinality(p_candidate_ids) or exists (
      ...
      where candidate.id is null
        ...
        or candidate.classroom_relevance_policy_version is distinct from (p_run ->> 'algorithmVersion')
  ...
  for candidate_row in
    ...
    where id = any(p_candidate_ids)
      ...
      and classroom_relevance_policy_version = (p_run ->> 'algorithmVersion')
...
begin
  ...
  if (
    ...
    where candidate.id = any(p_candidate_ids)
      ...
      and candidate.classroom_relevance_policy_version in ('classroom-use-v2', 'classroom-use-v3')
